<?php
    ...
    if ($_GET['idsupir']) {
        ...
        $cek1 = mysqli_query($conn, "SELECT * FROM tbl_sewa 
                                    ...
                                    WHERE id_supir='$idsupir' AND selesai=0
        ...
        if ($hit1 > 0) {
            ...
            $allData = array('status'=> 200,'message'=>'success','dataOrderSupir'=>$datas);
            $json = json_encode($allData, JSON_PRETTY_PRINT);
            echo $json;
        } else {
            $allData = array('status'=> 200,'message'=>'success','dataOrderSupir'=>[]);
            $json = json_encode($allData, JSON_PRETTY_PRINT);
            echo $json;
        }
    }